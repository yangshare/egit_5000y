...
.css_td {
  ...
  if(request.getParameter("insert")!=null){
    ...
    out.print("<script>alert('新增案例失败！');</script>");
    }
    if(request.getParameter("insert").equals("rename")){
    out.print("<script>alert('案例名重名，请重命名！');</script>");
    }
  }
  ...
  <body> 
   ...
   <td height="15" class="css_td">
           案例名：
   <input type="text" id="caseName" name="caseName" class="input_01" value=""/>
   </td>
   </tr>
 ...
 <img src="<%=basePath%>/admin/images/loading.gif"  id="loading" style="display: none;">
   ...
   /*  function mit(){
            document.getElementById("form2").action="UpLoad_execut";
             document.getElementById("form2").enctype="multipart/form-data";
    ...
    if(document.getElementById("typeText").value!=""){
       ...
       if(bool==true){
            if(caseName!="" && caseContent!=undefined){
              document.getElementById("form2").action="wCase_insert";
