...
.css_td {
  ...
  if(request.getParameter("update")!=null){
   ...
   out.print("<script>alert('成员更新失败！');</script>");
  }
  }
  ...
  usImpl=Alist.get(0);
 // pageContext.setAttribute("caseType",typesImpl.getTypeName());
   %>
  <body> 
   ...
   <td height="15" class="css_td">
           成员姓名：
   <input type="text" id="name" name="name" class="input_01" value="<%=usImpl.getName() %>"/>
   </td>
   </tr>
...
<input type="file" border="0" id="myFile" name="myFile" value="选择" size="5" onchange="javascript:mit();">
 <input type="hidden" name="usImg" value="<%=usImpl.getUsImg()%>">
  <br/><center>
 <img src="<%=basePath%>/admin/images/loading.gif"  id="loading" style="display: none;">
   ...
   <td height="60" class="css_td">
       成员介绍：
   <br/><br/><textarea name="content" id="textarea" class="input_01"><%=usImpl.getIntroduce()%></textarea>  
   </td>
   </tr>
     ...
     function submi(){
       ...
       var Name=document.getElementById("name").value;
            if(Name==""){
		       ...
		       document.getElementById("form2").action="wAbout_UsUpdate?id=<%=usImpl.getId()%>"
